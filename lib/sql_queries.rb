...
"select projects.title, (sum(pledges.amount) - projects.funding_goal) as amount_over_goal from projects, pledges where projects.id = pledges.project_id group by projects.title having amount_over_goal >= 0;"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"select users.name, sum(pledges.amount) as all_pledges from users, pledges where users.id = pledges.user_id group by users.name order by all_pledges, users.name;"
end
